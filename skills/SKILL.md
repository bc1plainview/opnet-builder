...
A comprehensive skill for building on Bitcoin Layer 1 with the OP_NET consensus protocol — trustless smart contracts on Bitcoin.
...
3. **Upload the `dist/` directory to IPFS using your preferred pinning service** (Pinata, web3.storage, Fleek, etc.)
...
- ❌ **Uploading files one by one** — use directory uploads with proper chunking
